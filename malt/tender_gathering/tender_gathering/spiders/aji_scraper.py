...
class AjiScraperSpider(scrapy.Spider):
    ...
    def choose_page(self, response):
        ...
        for page in range(1, 2): # max_page):
    ...
    def parse_details(self, response):
        ...
        if 'PS03' in type_produit:
          ...
          description = response.xpath('//div[contains(text(),"Description")]/following::div[1]/text()').extract_first()
          description = description.strip()
          criteria_list = response.xpath(u'//div[contains(text(),"Critères d\'attribution")]/following::div[1]/text()').extract_first()
          criteria_list = criteria_list.split() if not criteria_list is None else []
          sf = title.split() + description.split() + criteria_list
          ...
          items = {
            ...
            'N - CP': zip_code,
            'O - Nombre de lots': lot,
            'P - Pays': location,
            'Q - Dates du voyage': journey_date,
            'R - Ville/Région': destination,
            'S - Transport': transport,
            'T - Hébergement': slp_place,
            'U - Nombre d\'eleves': nb_people,
            'V - Accompagnateur(s)': nb_acc,
            'W - Durée du séjour': '',
            'X - Contact': contact_name,
            'Y': '',
            'Z - Telephone': contact_phone,
            'AA - Courriel': contact_mail,
          }
